...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    # Анимация "бот думает" - сообщения заменяют друг друга
    thinking_messages = [
    ...
    # Отправляем первое сообщение, затем редактируем его
    thinking_msg = await message.answer(thinking_messages[0])
    await asyncio.sleep(1.5)
    
    # Редактируем сообщение, заменяя текст
    for msg_text in thinking_messages[1:]:
        await thinking_msg.edit_text(msg_text)
        await asyncio.sleep(1.5)
