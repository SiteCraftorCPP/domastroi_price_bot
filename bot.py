...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        ...
        "О нас: 15 лет, 8 стран, 2000+ проектов, команда «одного окна» и очень качественные проекты без ошибок.\n\n"
        "Давайте рассчитаем стоимость вашего ремонта.\n"
        "Это займет всего 60 сек. и вы получите:\n\n"
...
async def process_consent(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        f"{user_name},\n\n"
        f"Ваш индивидуальный расчет стоимости почти готов!\n\n"
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    text = (
        "Отлично! Ваш номер принят ✅\n\n"
        "Наш менеджер свяжется с вами в течение 15 минут в рабочее время, "
        "чтобы озвучить расчет и согласовать время консультации дизайнера."
    )
