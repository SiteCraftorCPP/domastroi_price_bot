...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    sent_messages = []
    for i, msg_text in enumerate(thinking_messages):
        if i == 3:  # Сообщение 3.1 - отправляем вместе с предыдущим
            # Редактируем предыдущее сообщение, добавляя новую строку
            prev_msg = sent_messages[-1]
            combined_text = f"💬 Начинаю анализ ваших критериев\n{msg_text}"
            await prev_msg.edit_text(combined_text)
